...
from Lecture 9. The methods that were left with NotImplementedError have been
written here (s_path, welfare, evaluate), plus a couple of helpers used in
Questions 5 and 6.
...
class SolowModelClass:
    ...
    def s_path(self,s0,phi,s_inf=None):
        ...
        With s_inf=None the long-run level is par.s_bar, which is the rule in
        equation (6). Passing s_inf makes it the three-parameter rule from
        Question 5. The result is clipped to [0,1] so it is always a valid
        savings rate.
        ...
        Args:
            ...
            s_inf (float,optional): long-run savings rate, par.s_bar if None
    ...
    def welfare(self,c):
        """ the discounted sum of log(c_t), W = sum beta**t * log(c_t)

        Args:

            c (ndarray): the consumption path, par.T elements
    ...
    def evaluate(self,s0,phi,s_inf=None):
        """ welfare of the savings rule (s0,phi) (and optionally s_inf)

        Builds the savings path, simulates the economy and returns the welfare
        of the resulting consumption path. With s_inf=None this is the rule in
        equation (6); passing s_inf gives the three-parameter rule.

        Args:

            s0 (float): savings rate in period 0
            phi (float): speed of return to the long-run level
            s_inf (float,optional): long-run savings rate, par.s_bar if None

        Returns:

            (float): welfare W of the rule
